    ...
    <div className="crm-portal-page max-w-4xl">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <div className="space-y-5">
          ...
          <section className="card p-5"><h2 className="mb-3 text-sm font-medium">Tanda tangan</h2>{signatures.length > 0 && <ul className="mb-4 space-y-2">{signatures.map((signature) => <li key={signature.id} className="rounded bg-slate-50 p-2 text-xs"><strong>{signature.role}: {signature.signerName}</strong><span className="block text-slate-400">{formatDateTime(signature.signedAt)}</span>{signature.attachment && <a href={`/api/files/${signature.attachment.id}`} target="_blank" rel="noreferrer" className="mt-1 inline-block text-brand-600 hover:underline">Buka gambar tanda tangan</a>}</li>)}</ul>}<form action={signPickupAction} encType="multipart/form-data" className="space-y-3"><input type="hidden" name="recoveryId" value={recovery.id} /><input type="hidden" name="origin" value="portal" /><select name="role" className="input" defaultValue="CUSTOMER">{RECOVERY_SIGNATURE_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}</select><input name="signerName" className="input" placeholder="Nama penanda tangan" required /><label className="label" htmlFor="portal-signature-file">Gambar tanda tangan (opsional)</label><input id="portal-signature-file" name="signatureFile" type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" capture="environment" /><p className="text-xs text-slate-500">Nama tetap wajib. Gambar boleh ditambahkan jika tersedia.</p><button type="submit" className="btn-secondary w-full justify-center">Simpan tanda tangan</button></form></section>
        </div>
      </div>
    </div>
  );
