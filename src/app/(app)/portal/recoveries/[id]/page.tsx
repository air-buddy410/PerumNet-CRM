  ...
  return (
    <div className="crm-portal-page max-w-4xl">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
              ...
              <ul className="divide-y divide-slate-100">
                {recovery.items.map((item) => (
                  <li key={item.id} className="px-5 py-4">
                    ...
                    <RecoveryEvidencePanel action={attachEvidenceAction} recoveryId={recovery.id} kind="PICKUP" entityId={item.id} title="Bukti penarikan" items={serializeEvidence(itemEvidence.get(item.id) ?? [])} />
                  </li>
                ))}
              </ul>
            )}
        ...
        <div className="space-y-5">
          {!isClosed && <section className="card p-5"><h2 className="mb-3 text-sm font-medium">Catat kunjungan</h2><RecoveryAttemptForm action={recordAttemptAction} recoveryId={recovery.id} results={RECOVERY_ATTEMPT_RESULTS} /></section>}
          {!isClosed && pending.length > 0 && <section className="card p-5"><h2 className="mb-3 text-sm font-medium">Tarik perangkat</h2><RecoveryPickupForm action={pickupDevicesAction} recoveryId={recovery.id} items={pending} /></section>}
          {!isClosed && !recovery.physicalDisconnectedAt && <section className="card p-5"><h2 className="mb-2 text-sm font-medium">Konfirmasi pemutusan fisik</h2><p className="mb-3 text-xs text-slate-500">Konfirmasi setelah kabel/perangkat benar-benar dilepas di lokasi pelanggan.</p><form action={confirmDisconnectAction}><input type="hidden" name="recoveryId" value={recovery.id} /><button type="submit" className="btn-secondary w-full justify-center">Konfirmasi pemutusan</button></form></section>}
          <section className="card p-5"><h2 className="mb-3 text-sm font-medium">Tanda tangan</h2>{signatures.length > 0 && <ul className="mb-4 space-y-2">{signatures.map((signature) => <li key={signature.id} className="rounded bg-slate-50 p-2 text-xs"><strong>{signature.role}: {signature.signerName}</strong><span className="block text-slate-400">{formatDateTime(signature.signedAt)}</span></li>)}</ul>}<form action={signPickupAction} className="space-y-3"><input type="hidden" name="recoveryId" value={recovery.id} /><select name="role" className="input" defaultValue="CUSTOMER">{RECOVERY_SIGNATURE_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}</select><input name="signerName" className="input" placeholder="Nama penanda tangan" required /><button type="submit" className="btn-secondary w-full justify-center">Simpan tanda tangan</button></form></section>
        </div>
      </div>
    </div>
  );
