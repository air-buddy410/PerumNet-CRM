...
import { Activity, BookOpenCheck, Boxes, ChevronDown, ClipboardCheck, Contact, FolderKanban, Headphones, LayoutDashboard, LifeBuoy, Megaphone, RadioTower, Receipt, Server, Settings, UsersRound, Wallet, Wrench } from "lucide-react";
...
export function SidebarNav({ groups }: { groups: NavGroup[] }) {
  ...
  const iconFor = (group: string) => {
    ...
    if (group === "Helpdesk") return Headphones;
    if (group === "Billing") return Receipt;
    if (group === "Finance") return Wallet;
    if (group === "Akuntansi") return BookOpenCheck;
